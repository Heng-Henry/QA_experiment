...
def _apply_optional_mixer_rotation(vlm: nn.Module) -> None:
    mixer_rot_requested = _env_flag("COBRA_LLM_MIXER_HADAMARD", default=False)
    mixer_rot_require_success = _env_flag("COBRA_LLM_MIXER_REQUIRE_SUCCESS", default=True)
    if not mixer_rot_requested:
        return
    ...
    try:
        ...
        traceback.print_exc()
        if mixer_rot_require_success:
            raise RuntimeError(
                "Mixer rotation was requested (COBRA_LLM_MIXER_HADAMARD=1) but failed. "
                "To continue without rotation, set COBRA_LLM_MIXER_REQUIRE_SUCCESS=0."
            ) from e
        print("[MixerRotation] WARNING: continuing without mixer rotation (COBRA_LLM_MIXER_REQUIRE_SUCCESS=0).")
...
def load_quantized_cobra_vlm(
    *,
    ...
    if w_bits is None:
        ...
        _apply_optional_mixer_rotation(vlm)
        return vlm
