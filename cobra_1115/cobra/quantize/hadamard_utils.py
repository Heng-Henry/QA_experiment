...
try:
    import fast_hadamard_transform  # type: ignore
    _FAST_HADAMARD_AVAILABLE = True
    _FAST_HADAMARD_IMPORT_ERROR = None
except Exception as _e:  # pragma: no cover - environment dependent
    fast_hadamard_transform = None  # type: ignore
    _FAST_HADAMARD_AVAILABLE = False
    _FAST_HADAMARD_IMPORT_ERROR = _e
...
def matmul_hadU_cuda(X, hadK, K):
    ...
    if not _FAST_HADAMARD_AVAILABLE:
        raise ImportError(
            "fast_hadamard_transform is unavailable; CUDA hadamard path cannot run. "
            "Use matmul_hadU (torch fallback) or install a compatible fast_hadamard_transform build."
        ) from _FAST_HADAMARD_IMPORT_ERROR
    if K == 1:
...
def apply_exact_had_to_linear(module, had_dim=-1, output=False, down_proj_dim=1, partial_o = False):
    ...
    if had_dim == -1:
        ...
        if output:
            ...
            if not _FAST_HADAMARD_AVAILABLE:
                raise ImportError(
                    "fast_hadamard_transform is unavailable; apply_exact_had_to_linear(had_dim!=-1, output=True) "
